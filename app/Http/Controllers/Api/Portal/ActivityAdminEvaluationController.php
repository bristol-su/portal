...
class ActivityAdminEvaluationController extends Controller
{

    public function user(ActivityRepositoryContract $activityRepository, Authentication $authentication, Activity $activity)
    {
        if(!LogicTester::evaluate($activity->adminLogic, $authentication->getUser())) {
    ...
    {
        if(!LogicTester::evaluate($activity->adminLogic, $authentication->getUser(), $authentication->getGroup())) {
            throw new AuthorizationException('You are not a part of the ' . $activity->name . ' activity.');
        }
    ...
    {
        if(!LogicTester::evaluate($activity->adminLogic, $authentication->getUser(), $authentication->getGroup(), $authentication->getRole())) {
            throw new AuthorizationException('You are not a part of the ' . $activity->name . ' activity.');
        }
